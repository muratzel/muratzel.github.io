    ...
    query.find(
        {
            success: function (tutorials) {
                for (var i = 0; i < tutorials.length; i++) {
                    mainPageTutorialsDisplayUl.append("<li class='list-group-item' onclick = 'populateModal(this);' id='" + tutorials[i].id + "'><h3>" + tutorials[i].get('title') + "<small class='col-xs-offset-1'>" + tutorials[i].get('type') + "</small><small class='col-xs-offset-1 pull-right>" + tutorials[i].get('rating') + "(" + tutorials[i].get('voters') + " voters)</small></h3></li>");
                }
            },
...
addTutorialButton.click(function () {
    ...
    newTutorial.save(null,
        {
            success: function (tutorial) {
                mainPageTutorialsDisplayUl.append("<li class='list-group-item' onclick = 'populateModal(this);' id='" + tutorial.id + "'><h3>" + tutorial.get('title') + "<small class='col-xs-offset-1'>" + tutorial.get('type') + "</small><small class='col-xs-offset-1 pull-right>"+tutorial.get('rating')+"("+tutorial.get('voters')+" voters)</small></h3></li>");
            },
            error: function (tutorial, error) {
            }
        }
